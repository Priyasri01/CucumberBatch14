...
public class APIWorkFlowSteps {
    ...
    public static String employee_id; // created the variable in global level
    ...
    public void the_employee_id_is_stored_as_global_to_be_used_for_other_request(String empId) {
        employee_id = response.jsonPath().getString(empId); //this gives us the value of the empId right
    ...
    public void the_retrieved_data_at_object_should_match_with_the_data_used_for_creating_the_employee
        ...
        for(Map<String, String> map : expectedData){ // expectedDate contains list of map so we are created map to retrive one map at time and we are doing verification part
    ...
    @Given("a request is prepared for update an employee with dynamic data {string},{string} , {string}  , {string} , {string} , {string} , {string} , {string}")
    public void a_request_is_prepared_for_update_an_employee_with_dynamic_data(String emp_id, String firstname, String lastname, String middlename, String gender, String  dob, String empStatus, String jobTitle) {
        request = given().
                header(APIConstants.Header_Key_Content_Type, APIConstants.Header_Value_Content_Type).
                header(APIConstants.Header_Key_Authorization, GenerateTokenSteps.token).
                body(APIPayloadConstant.updateEmployeePayloadDynamic(emp_id,firstname, lastname, middlename, gender,
                        dob, empStatus, jobTitle));
    }



    @When("a Put call is made to update employee deatils")
    public void a_put_call_is_made_to_update_employee_deatils() {
        response=request.when().put(APIConstants.UPDATE_EMPLOYEE_URI);
        response.prettyPrint();
    }
    @Then("the status code for updata  an employee is {int}")
    public void the_status_code_for_updata_an_employee_is(Integer int1) {
       response.then().assertThat().statusCode(int1);
    }
