...
#to generate the token for all the request, we kept in here in background
 ...
 @api
  Scenario: Getting the created employee
   ...
   And the retrieved data at "employee" object should match with the data used for creating the employee
    ...
    |ravi        |ragual        |ms             |Female    |1970-01-12  |confirmed |QA Engineer  |
 ...
 @apiupdate
 Scenario: API test case for updatating the employee data  using highly dynamic body
  Given a request is prepared for update an employee with dynamic data "50541A","Ram" , "Ravi"  , "ms" , "M" , "1970-01-12" , "Permanent Employee" , "QA Team Lead"
  When a Put call is made to update employee deatils
  Then the status code for updata  an employee is 200
